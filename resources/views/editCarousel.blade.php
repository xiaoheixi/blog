...
<body>
    ...
    <h1 class="pageTitle">Fill in the form to edit a carousel below.</h1>
    <form action="/carousel/{{ $carouselContent->carouselName }}" method="post" enctype='multipart/form-data'>
        ...
        <input type="text" id="carouselName" name="carouselName" autocomplete="off"
            value="{{ $carouselContent -> carouselName }}"><br>
        @error('carouselName') <p style="color: red">{{ $message }}</p> @enderror
        <label for="carouselImage">Carousel Image:</label><br>
        <input type="file" id="carouselImage" name="carouselImage" autocomplete="off"
            value="{{ $carouselContent -> carouselImage }}"><br>
